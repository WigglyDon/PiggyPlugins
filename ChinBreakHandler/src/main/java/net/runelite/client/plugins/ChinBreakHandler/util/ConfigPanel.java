...
//            }
    ...
    for (ConfigItemDescriptor cid : pluginConfig.getItems()) {
      ...
      if (!name.isEmpty()) {
        ...
        configEntryName.setToolTipText(
            "<html>" + name + ":<br>" + cid.getItem().description() + "</html>");
      ...
      if (cid.getType() == boolean.class) {
        ...
        checkbox.setSelected(Boolean.parseBoolean(
            configManager.getConfiguration(pluginConfig.getGroup().value(),
                cid.getItem().keyName())));
      ...
      if (cid.getType() == int.class) {
        int value = Integer.parseInt(configManager.getConfiguration(pluginConfig.getGroup().value(),
            cid.getItem().keyName()));
      ...
      if (cid.getType() instanceof Class && ((Class<?>) cid.getType()).isEnum()) {
        ...
        JComboBox box = new JComboBox<Enum<?>>(
            type.getEnumConstants()); // NOPMD: UseDiamondOperator
        ...
        try {
          Enum<?> selectedItem = Enum.valueOf(type,
              configManager.getConfiguration(pluginConfig.getGroup().value(),
                  cid.getItem().keyName()));
  ...
  private void changeConfiguration(Component component, ConfigDescriptor cd,
    ...
    if (component instanceof JCheckBox) {
      ...
      configManager.setConfiguration(cd.getGroup().value(), cid.getItem().keyName(),
          ((Enum) jComboBox.getSelectedItem()).name());
    }
