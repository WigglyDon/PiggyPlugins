...
public class LavaRunecrafterPlugin extends Plugin {
  ...
  public void onGameTick(GameTick event) {
    ...
    if (hadbook != null) {
      if (!Widgets.search().withTextContains("What do you want?").hiddenState(false).empty()
          || !Widgets.search().withTextContains("Can you repair").hiddenState(false).empty()) {
    ...
    if (bankChest != null) {
      if (client.getWidget(WidgetInfo.BANK_CONTAINER) == null) {
        try {
          binding = client.getItemContainer(InventoryID.EQUIPMENT)
              .getItem(EquipmentInventorySlot.AMULET.getSlotIdx());
      ...
      if (binding == null) {
        ...
        Widget bindingNeck = EthanApiPlugin.getItem(ItemID.BINDING_NECKLACE,
        ...
        MousePackets.queueClickPacket();
        WidgetPackets.queueWidgetActionPacket(9,
            WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER.getPackedId(), ItemID.BINDING_NECKLACE,
            freeSlot);
        bindingCharges = bindingCharges == -1 ? bindingCharges : 16;
      }
      if (EthanApiPlugin.getItem(ItemID.EARTH_RUNE, WidgetInfo.INVENTORY) == null) {
        EthanApiPlugin.stopPlugin(this);
        client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "Out of earth runes",
            "LavaRunecrafterPlugin");
      }
      handleStamina();
      handleDueling();
      ...
      switch (config.TeleMethod()) {
        case RING_OF_ELEMENTS:
          Widget ring = EthanApiPlugin.getItem(ItemID.RING_OF_THE_ELEMENTS_26818,
              WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER);
          if (ring == null) {
            client.addChatMessage(ChatMessageType.GAMEMESSAGE, "",
                "ring of the elements tele enabled but" +
                    " ring not found",
                "LavaRunecrafterPlugin");
            EthanApiPlugin.stopPlugin(this);
            return;
          ...
          MousePackets.queueClickPacket();
          WidgetPackets.queueWidgetActionPacket(6, 9764864, ItemID.RING_OF_THE_ELEMENTS_26818,
              ring.getIndex());
          break;
  ...
  public boolean pouchesDegraded() {
    return EthanApiPlugin.getItemFromList(
        new int[]{ItemID.MEDIUM_POUCH_5511, ItemID.LARGE_POUCH_5513, ItemID.GIANT_POUCH_5515,
            ItemID.COLOSSAL_POUCH_26786}, WidgetInfo.INVENTORY) != null;
  }

  public void handleDueling() {
    Widget dueling = EthanApiPlugin.getItemFromList(
        new int[]{ItemID.RING_OF_DUELING1, ItemID.RING_OF_DUELING2,
            ItemID.RING_OF_DUELING3, ItemID.RING_OF_DUELING4, ItemID.RING_OF_DUELING5,
            ItemID.RING_OF_DUELING6, ItemID.RING_OF_DUELING7, ItemID.RING_OF_DUELING8},
        WidgetInfo.BANK_ITEM_CONTAINER);
    int equippedDueling = EthanApiPlugin.checkIfWearing(
        new int[]{ItemID.RING_OF_DUELING1, ItemID.RING_OF_DUELING2,
            ItemID.RING_OF_DUELING3, ItemID.RING_OF_DUELING4, ItemID.RING_OF_DUELING5,
    ...
    if (config.TeleMethod() == TeleportMethods.RING_OF_DUELING) {
      dueling = EthanApiPlugin.getItemFromList(
          new int[]{ItemID.RING_OF_DUELING2, ItemID.RING_OF_DUELING4,
    ...
    if (dueling == null) {
      EthanApiPlugin.stopPlugin(this);
      client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "Out of dueling rings",
          "LavaRunecrafterPlugin");
    }
    MousePackets.queueClickPacket();
    ...
    MousePackets.queueClickPacket();
    WidgetPackets.queueWidgetActionPacket(9,
        WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER.getPackedId(), dueling.getItemId(), freeSlot);
  }
  ...
  public void handleStamina() {
    ...
    if (stamina == null || freeSlot == -1) {
      EthanApiPlugin.stopPlugin(this);
      client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "Out of stamina potions",
          "LavaRunecrafterPlugin");
    }
    ...
    MousePackets.queueClickPacket();
    WidgetPackets.queueWidgetActionPacket(9,
        WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER.getPackedId(), ItemID.STAMINA_POTION1, freeSlot);
    if (config.VialSmasher()) {
      return;
    }
    MousePackets.queueClickPacket();
    WidgetPackets.queueWidgetActionPacket(2,
  ...
  public void handlePouches() {
    ...
    essence = essence == null ? EthanApiPlugin.getItem(ItemID.RUNE_ESSENCE,
        WidgetInfo.BANK_ITEM_CONTAINER) : essence;
    if (essence == null) {
      EthanApiPlugin.stopPlugin(this);
      client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "Out of essence",
          "LavaRunecrafterPlugin");
    }
    int essenceSlots =
        EthanApiPlugin.getEmptySlots(WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER) + getEssenceSlots(
            WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER);
    while (essenceNeeded() > 0) {
      ...
      for (Widget pouch : pouches.keySet()) {
        ...
        if (EthanApiPlugin.getItem(getAlternative(pouch), WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER)
            != null) {
          MousePackets.queueClickPacket();
          WidgetPackets.queueWidgetAction(EthanApiPlugin.getItem(getAlternative(pouch),
              WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER), "Fill");
        } else {
          MousePackets.queueClickPacket();
    ...
    Widget lavaRunes = EthanApiPlugin.getItem(ItemID.LAVA_RUNE,
        WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER);
    if (lavaRunes != null) {
  ...
  public HashMap<Widget, int[]> getPouches() {
    pouches.clear();
    pouches.put(
        EthanApiPlugin.getItem(ItemID.SMALL_POUCH, WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER),
        new int[]{0, 3});
    pouches.put(
        EthanApiPlugin.getItem(ItemID.MEDIUM_POUCH, WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER),
        new int[]{0, 6});
    pouches.put(
        EthanApiPlugin.getItem(ItemID.LARGE_POUCH, WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER),
        new int[]{0, 9});
    pouches.put(
        EthanApiPlugin.getItem(ItemID.GIANT_POUCH, WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER),
        new int[]{0, 12});
    pouches.put(
        EthanApiPlugin.getItem(ItemID.COLOSSAL_POUCH, WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER),
        new int[]{0, 40});
    pouches.remove(null);
    return pouches;
  }
  ...
  public int getEssenceSlots(WidgetInfo widgetInfo) {
    List<Widget> inventoryItems = Arrays.asList(
        client.getWidget(widgetInfo.getId()).getDynamicChildren());
    return (int) inventoryItems.stream()
        .filter(item -> item.getItemId() == ItemID.PURE_ESSENCE || item.getItemId()
  ...
  public void onChatMessage(ChatMessage e) {
    ...
    if (e.getMessage().contains("left before your Binding necklace disintegrates.")) {
      bindingCharges = Integer.parseInt(
          e.getMessage().split("You have ")[1].replace(" charges left before your" +
              " Binding necklace" +
              " disintegrates.", ""));
    }
  }
