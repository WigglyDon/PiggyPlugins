...
public class DialogueContinuerPlugin extends Plugin {
  ...
  private void onGameTick(GameTick event) {
    ...
    Inventory.search().onlyUnnoted().withName("Vial").filter(v -> config.dropEmptyVials()).first()
        .ifPresent(vial -> {
          InventoryInteraction.useItem(vial, "Drop");
        });

    Inventory.search().onlyUnnoted().withName("Jug").filter(b -> config.dropEmptyWineJugs()).first()
        .ifPresent(bone -> {
  ...
  private void dismissRandoms() {
    NPCs.search().alive().walkable().withAction("Dismiss").interactingWithLocal().first()
        .ifPresent(npc -> {
//            log.info("Dismissing random: " + npc.getName());
  ...
  private boolean continueDialogue() {
    Optional<Widget> questHelperOpt = Widgets.search().withParentId(14352385).withTextContains("[")
        .withTextContains("]").first();
...
//            timeout = 0;
//        });
    Optional<Widget> mainContinueOpt = Widgets.search().withTextContains("Click here to continue")
        .first();
